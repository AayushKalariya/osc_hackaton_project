...
if 'quick_action' in st.session_state:
    ...
    # Clear the quick action after setting default
...
# Add Medication Page
    ...
    with st.form("add_medication"):
        ...
        with col1:
            ...
            # Input for how many times per day
        ...
        with col2:
            ...
            if doses_per_day >= 1:
                ...
                # Dynamic creation of time inputs based on doses_per_day
                all_times_set = True
                for i in range(doses_per_day):
                    ...
                    # Only add time if user has actually selected one
                    if dose_time is not None:
                        times.append(dose_time.strftime("%H:%M"))
                    else:
                        all_times_set = False

                # Show the complete schedule only if all times are set
        ...
        if submitted:
            # Check that required fields are filled and times were collected for all doses
            if med_name and dosage and len(times) == doses_per_day and all(times):
                # Sort times chronologically for a clean schedule display
                times.sort()
                
                # Create frequency label based on number of doses
...
# Manage Medications Page
                        ...
                        if st.session_state.get(f'confirm_delete_{med_id}', False):
                            ...
                            with col_confirm:
                                if st.button("✅ Confirm Delete", key=f"confirm_del_{med_id}", type="primary"):
                                    # Delete associated side effects
            ...
            for med_id, med in archived_meds.items():
                with st.expander(f"📦 **{med['name']}** ({med['dosage']}) - ARCHIVED"):
                    ...
                    with col2:
                        ...
                        # Delete confirmation for archived meds
    ...
    with tab3:
        ...
        # Archive all completed medications
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            if st.button("📁 Archive Completed Medications", help="Archive medications that no longer need to be taken"):
                # This is now a manual process since we don't have logs to track adherence
...
# Side Effects Page
    ...
    with col1:
        ...
        if not active_meds:
            ...
            # FIX: Create a map of display names to medication IDs for correct selection
            med_options_map = {
            ...
            with st.form("side_effect_log"):
                ...
                # Get the actual med_id using the map
    ...
    with col2:
        ...
        if tracker.side_effects:
            for effect in sorted(tracker.side_effects, key=lambda x: x['timestamp'], reverse=True)[:10]:
                # FIX: Use .get() to safely retrieve medication name
...
# Analytics Page
    ...
    if tracker.side_effects:
        ...
        # Medication side effect breakdown
        st.subheader("💊 Medication Side Effect Analysis")
        
        med_analysis = {}
        for effect in tracker.side_effects:
            med_id = effect['med_id']
            # Improved safety in retrieving med_name
    ...
    # Mood Analytics (if mood data exists)
    if tracker.mood_logs:
        st.subheader("😊 Mood Analytics")
        
        # Get recent mood data manually
...
# st.rerun() can be called periodically or on specific events
